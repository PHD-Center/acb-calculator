...
- **一頁式報表列印 / PDF 匯出**:評估完成後,右上角「🖨️ 列印 / 匯出 PDF」一鍵叫出瀏覽器原生列印對話框,以 `@media print` CSS 排版成 A4 一頁(用藥清單、6 量表分數、三項風險指標、警示與替代建議、文獻引用 + disclaimer),選擇「另存為 PDF」即得精美病歷報表;Windows 用戶可直接「Microsoft Print to Word」匯出 .docx
